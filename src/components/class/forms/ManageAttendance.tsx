...
export default function ManageAttendance({ classId, eventId }: ManageAttendanceProps) {
    ...
    const updateAttendanceMutation = trpc.attendance.update.useMutation({
        onSuccess: (data) => {
            ...
            emitAttendanceUpdate(classId, data);
        },
        onError: (error) => {
            dispatch(addAlert({ 
    ...
    }, [attendanceLoading, attendanceData, classId, eventId, students]);
    ...
    const updateAttendance = async (studentId: string, status: 'present' | 'late' | 'absent') => {
        ...
        setAttendance(newAttendance as AttendanceRecord);
    ...
    return (
        <div className="flex flex-col space-y-6 w-[30rem]">
            ...
            <div className="w-full border border-border dark:border-border-dark rounded-lg p-4 shadow-md bg-background dark:bg-background-subtle">
                ...
                <div className="grid grid-cols-[3fr_1fr_1fr_1fr_auto] gap-4 mb-3 font-medium px-4 text-foreground-secondary">
                    <span>Student</span>
                ...
                <div className="space-y-2">
                    {allStudents.map((studentId) => {
                        ...
                        // Determine the primary status (priority: present > late > absent)
                        const isPresent = attendance.present.some((s: { id: string }) => s.id === studentId);
                        const isLate = attendance.late.some((s: { id: string }) => s.id === studentId);
                        const isAbsent = attendance.absent.some((s: { id: string }) => s.id === studentId);
                        
                        // Only check the highest priority status
                        const checkedPresent = isPresent;
                        const checkedLate = !isPresent && isLate;
                        const checkedAbsent = !isPresent && !isLate && isAbsent;
                        
                        return (
                            <div key={studentId} 
                                className="grid grid-cols-[3fr_1fr_1fr_1fr_auto] gap-4 px-4 py-3 items-center rounded-md hover:bg-background-muted dark:hover:bg-background-subtle transition-colors"
                            >
                                ...
                                <button
                                    ...
                                    className="text-foreground-muted hover:text-error transition-colors p-1 rounded"
                                >
